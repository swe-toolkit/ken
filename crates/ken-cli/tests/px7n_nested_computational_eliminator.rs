...
#[test]
#[ignore = "RT-DUPLICATED-RESPONSE-BLOCK: re-measured individually at cf4533b746; this row still stops first at two host response cases claim one operation constructor. The collision is a false alarm over two legitimate per-arm-inlined copies of the prelude dispatcher, but neither occurrence-keying nor relaxation is authorized: producer_call_origin still has production USE sites, with observables flat or unreached only on this four-row population. Clause 3 was struck by Architect ruling evt_7sj5xmgcxwk5f; no Vis in this program selects any copied route. With the construction refusal alone suppressed, this same program next stops at RT-FRAME-MARKER-ONCE: checked Runtime frame marker was consumed more than once. Stays ignored until the duplicate-response guard replacement and frame-marker repair are both settled."]
