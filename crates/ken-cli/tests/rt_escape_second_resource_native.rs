...
#[test]
#[ignore = "RT-DUPLICATED-RESPONSE-BLOCK, readmission half measured at 9dfa6978e; the census and the refusal attribution are carried from RT-HOST-RESPONSE-ROUTE-KEY-COLLISION measured at 89dc3b0e5 on THIS ROW and re-measured unchanged here. This row builds rt_escape_escape_file_then_readat -- the two rows in this file build DIFFERENT Ken programs and no census value transfers between them. THIS ROW: refuses at the planner invariant two host response cases claim one operation constructor, NOT at the mechanism named below; the constructor it SELECTS is rt_escape_escape_file_then_readat::FSOp::ctor_543, and that selected constructor is itself one of the collisions counted below, which is why deferring the refusal to the point of use does NOT clear this row. THIS PROGRAMS PLAN, census run to completion and counted by the probe itself rather than by grep: 29 colliding constructors, all 29 agreeing on their operation, effect-origin deltas a single distinct value of 317. A constant offset across the whole set is a duplicated block, not 29 competing claims. The count is of COLLISIONS OBSERVED IN THIS PLAN; it is not a claim about how many host-operation constructors exist. A SECOND BLOCKER IS NOW SEEN, measured at 9dfa6978e: with the construction-time refusal suppressed and nothing else changed, this row advances to ComputationalMatch: tree-producing match scrutinee is not Bool or a constructor. WHETHER RT-CLOSURE-BOUNDARY-LANE IS ALSO A BLOCKER REMAINS UNDETERMINED -- the refusal now reached is a different mechanism, and nothing has seen past IT, so the durable-lane claim is still neither confirmed nor refuted. CENSUS RE-MEASURED at 9dfa6978e and UNCHANGED: 29 colliding constructors, all 29 agreeing on their operation, effect-origin deltas a single value of 317. That is PER INVOCATION of host_response_routes, which runs TWICE in this compile, so a line count over the probe's output reads 58 where the census reads 29; the two numbers measure different things and neither is wrong. DELETING THE CHECK IS NOT THE REPAIR, measured: with only the refusal suppressed, each invocation performs 29 silent overwrites, dropping 29 constructors' wiring coordinates, and NOTHING refuses -- not at construction, and not at the point of use, where selected_host_response_route's own one Vis operation subtree selects more than one host response producer does not fire. The last-written route is simply used. AND THE TWO PROGRAMS IN THIS FILE ARE NOT INDEPENDENT WITNESSES, measured at 9dfa6978e: they were censused separately and no census value transfers, but EVERY proc of rt_escape_escape_file_then_readat also appears in rt_escape_nat_fanout_escaped -- after_file_escape, handle_outer and main byte-identical, read_body differing in four lines -- so two agreeing censuses across these two rows are ONE shape seen twice, not two independent replications. RULED by the Architect, 2026-09-18, and it is NEITHER of the two arms this label previously offered. THE UNIQUENESS INVARIANT IS CORRECT, and responses.rs:1279 is the guard between here and a SILENT WRONG-CONTINUATION ROUTE rather than a construction-time tidiness assertion. The direct evidence is the suppression measurement stated just above: the two colliding copies agree on their operation and differ on all THREE origins, which are exactly the fields naming WHICH producer and WHICH continuation the response reaches, so the copies are NOT interchangeable and last-insert-wins sends some Vis sites to the wrong one. De-duplicating is rejected as well: per-arm inlining of a shared callee is a legitimate transformation, the invariant is a claim about the SOURCE dispatcher, and plan.source_occurrences is POST-inlining -- the subject is wrong, not the claim. And re-keying the producer alone is the dangerous direction, because the consumer at selected_host_response_route still looks up by constructor: the collision would disappear while the mis-route did not. THE REPAIR IS A THIRD THING, larger than either rejected arm -- producer and consumer move TOGETHER, the consumer selecting by the same key the producer inserted under, and a Vis site whose copy is absent REFUSING rather than falling back to another copy. RT-HOST-RESPONSE-OCCURRENCE-KEY is ONE PART of that repair and is NOT sufficient alone. READMITS when that third-answer repair lands under its three-clause acceptance bar; re-measure for further blockers at that point. Until it lands this row's owner is RT-DUPLICATED-RESPONSE-BLOCK. Base 21fd46dc in the prior label was stale and is dropped."]
...
// Annotation only -- test body and expectations are unchanged.
#[test]
#[ignore = "RT-DUPLICATED-RESPONSE-BLOCK, readmission half measured at 9dfa6978e; the census and the refusal attribution are carried from RT-HOST-RESPONSE-ROUTE-KEY-COLLISION measured at 89dc3b0e5 on THIS ROW and re-measured unchanged here. This row builds rt_escape_nat_fanout_escaped -- the two rows in this file build DIFFERENT Ken programs and no census value transfers between them. THIS ROW: refuses at the planner invariant two host response cases claim one operation constructor, NOT at the mechanism named below; the constructor it SELECTS is rt_escape_nat_fanout_escaped::FSOp::ctor_543, and that selected constructor is itself one of the collisions counted below, which is why deferring the refusal to the point of use does NOT clear this row. THIS PROGRAMS PLAN, census run to completion and counted by the probe itself rather than by grep: 29 colliding constructors, all 29 agreeing on their operation, effect-origin deltas a single distinct value of 317. A constant offset across the whole set is a duplicated block, not 29 competing claims. The count is of COLLISIONS OBSERVED IN THIS PLAN; it is not a claim about how many host-operation constructors exist. A SECOND BLOCKER IS NOW SEEN, measured at 9dfa6978e: with the construction-time refusal suppressed and nothing else changed, this row advances to ComputationalMatch: tree-producing match scrutinee is not Bool or a constructor. WHETHER RT-CLOSURE-BOUNDARY-LANE IS ALSO A BLOCKER REMAINS UNDETERMINED -- the refusal now reached is a different mechanism, and nothing has seen past IT, so the durable-lane claim is still neither confirmed nor refuted. CENSUS RE-MEASURED at 9dfa6978e and UNCHANGED: 29 colliding constructors, all 29 agreeing on their operation, effect-origin deltas a single value of 317. That is PER INVOCATION of host_response_routes, which runs TWICE in this compile, so a line count over the probe's output reads 58 where the census reads 29; the two numbers measure different things and neither is wrong. DELETING THE CHECK IS NOT THE REPAIR, measured: with only the refusal suppressed, each invocation performs 29 silent overwrites, dropping 29 constructors' wiring coordinates, and NOTHING refuses -- not at construction, and not at the point of use, where selected_host_response_route's own one Vis operation subtree selects more than one host response producer does not fire. The last-written route is simply used. AND THE TWO PROGRAMS IN THIS FILE ARE NOT INDEPENDENT WITNESSES, measured at 9dfa6978e: they were censused separately and no census value transfers, but EVERY proc of rt_escape_escape_file_then_readat also appears in rt_escape_nat_fanout_escaped -- after_file_escape, handle_outer and main byte-identical, read_body differing in four lines -- so two agreeing censuses across these two rows are ONE shape seen twice, not two independent replications. RULED by the Architect, 2026-09-18, and it is NEITHER of the two arms this label previously offered. THE UNIQUENESS INVARIANT IS CORRECT, and responses.rs:1279 is the guard between here and a SILENT WRONG-CONTINUATION ROUTE rather than a construction-time tidiness assertion. The direct evidence is the suppression measurement stated just above: the two colliding copies agree on their operation and differ on all THREE origins, which are exactly the fields naming WHICH producer and WHICH continuation the response reaches, so the copies are NOT interchangeable and last-insert-wins sends some Vis sites to the wrong one. De-duplicating is rejected as well: per-arm inlining of a shared callee is a legitimate transformation, the invariant is a claim about the SOURCE dispatcher, and plan.source_occurrences is POST-inlining -- the subject is wrong, not the claim. And re-keying the producer alone is the dangerous direction, because the consumer at selected_host_response_route still looks up by constructor: the collision would disappear while the mis-route did not. THE REPAIR IS A THIRD THING, larger than either rejected arm -- producer and consumer move TOGETHER, the consumer selecting by the same key the producer inserted under, and a Vis site whose copy is absent REFUSING rather than falling back to another copy. RT-HOST-RESPONSE-OCCURRENCE-KEY is ONE PART of that repair and is NOT sufficient alone. READMITS when that third-answer repair lands under its three-clause acceptance bar; re-measure for further blockers at that point. Until it lands this row's owner is RT-DUPLICATED-RESPONSE-BLOCK. Base 21fd46dc in the prior label was stale and is dropped."]
