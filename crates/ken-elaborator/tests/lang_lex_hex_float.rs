...
fn t(s: &str) -> Token {
    Lexer::lex(s).unwrap().into_iter().next().unwrap().0
}
...
fn hex_float_values_and_boundaries() {
    ...
    assert_eq!(
        t("0x100000000000009p-56"),
        Token::FloatLit(1.0000000000000002)
    );
    assert_eq!(
        t("0x100000000000018p-56"),
        Token::FloatLit(1.0000000000000004)
    );
    assert_eq!(
        t("0x1p-1022"),
        Token::FloatLit(f64::from_bits(0x0010_0000_0000_0000))
    );
    assert_eq!(
        t("0x1p-1023"),
        Token::FloatLit(f64::from_bits(0x0008_0000_0000_0000))
    );
    ...
    assert!(matches!(
        Lexer::lex("0xFF:x.1").unwrap()[0].0,
        Token::Nat(255)
    ));
    let plus: Vec<_> = Lexer::lex("0xFF+p")
        .unwrap()
        .into_iter()
        .map(|(t, _)| t)
        .collect();
    ...
    let minus: Vec<_> = Lexer::lex("0xFF-p")
        .unwrap()
        .into_iter()
        .map(|(t, _)| t)
        .collect();
