...
/// THE GAP: this fixture reaches one error exit; the production closure
/// brackets all `?`-bearing method construction, while a debug assertion
/// checks the restored depth on both success and error. Removing the
/// unconditional restore makes this unchanged fixture return the prior
