...
fn separators_and_radix_integers_have_boundaries() {
    ...
    assert_eq!(
        tokens("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"),
        vec![Token::IntLit(wide), Token::Eof]
    );
...
fn exponent_values_are_not_zeroed() {
    assert_eq!(
        tokens("3.14e5"),
        vec![Token::FloatLit(314000.0), Token::Eof]
    );
...
fn decimal_fraction_separators_do_not_change_exponent() {
    assert_eq!(
        tokens("1.0_0d"),
        vec![Token::DecimalLit(BigInt::from(100), -2), Token::Eof]
    );
}
