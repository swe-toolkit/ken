...
pub(crate) struct CheckedHostSpineV1 {
    ...
    pub resource_revoked: StableSymbol,
    ...
    pub read_chunk: StableSymbol,
    pub read_result_eof: StableSymbol,
...
fn lower_checked_host_computation(
    ...
    if let Some((constructor, args)) = constructor_application_spine(term) {
        if constructor.symbol == spine.ret {
            let value_index = args.len().checked_sub(1).ok_or_else(|| {
            ...
            let mut value_path = path.to_vec();
            value_path.extend([4, value_index as u64]);
...
mod px7l_tests {
    ...
    fn spine() -> CheckedHostSpineV1 {
        ...
        CheckedHostSpineV1 {
            ...
            resource_revoked: StableSymbol::constructor(
                &family("ResourceError"),
                "ResourceRevoked",
            ),
            ...
            read_chunk: StableSymbol::constructor(&family("ReadResult"), "Chunk"),
            read_result_eof: StableSymbol::constructor(&family("ReadResult"), "Eof"),
...
mod d1b_role_b_decoder_alignment {
    ...
    fn every_role_decodes_into_the_field_it_was_encoded_from() {
        let spine = CheckedHostSpineV1 {
            ...
            resource_revoked: sentinel("resource_revoked"),
            ...
            read_chunk: sentinel("read_chunk"),
            read_result_eof: sentinel("read_result_eof"),
            unit: sentinel("unit"),
        ...
        (spine.resource_revoked.as_str(), "resource_revoked"),
