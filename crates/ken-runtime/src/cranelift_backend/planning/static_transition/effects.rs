...
///
/// **Total over the 22 admitted operations, with no `_` arm**, so a new
...
fn host_effect_seat_contract(
    ...
    // population is the exact complement: the eleven FS-path operations require
    ...
    match (operation, ordinal) {
        ...
        // PROVED carried, per seat: the real-artifact measurements observed a
        // carried word reaching each and the existing span observer consuming
        // it. These are direct operation consumers, not site-bound uses.
        (Op::ConsoleWrite, 1)
        | (Op::FsWriteFile, 2)
        | (Op::FsAppendFile, 1)
        ...
        | (Op::FsAppendFile, 0)
        | (Op::FsMetadata, 0)
        | (Op::FsReadDirectory, 0)
        | (Op::FsRemoveFile, 0)
        | (Op::FsRename, 0)
        | (Op::FsChangeMode, 0)
        | (Op::FsOpen, 0) => Some(bytes),
        (Op::FsCreateDirectory, 0) | (Op::FsRemoveDirectory, 0) => Some(tag),
        (Op::FsCreateDirectory, 1) | (Op::FsRemoveDirectory, 1) => Some(bytes),
