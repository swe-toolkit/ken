...
pub(super) enum EffectSeatDispatchMutation {
    ...
    /// Misclassify a host-reply-generated Bytes child as a scalar. This covers
    /// ConsoleRead's chunk and each ABI-A3 directory-entry name; the recipe/
    /// argument reconciliation must reject the wrong source kind.
    HostResponseAsScalar,
...
const IO_ERROR_INVALID_INPUT_DISCRIMINATOR: i64 = 6;
...
impl<'a> Lowering<'a> {
    ...
    /// `List DirEntry` result. Validation consumes the entire payload before
    /// any response-derived aggregate is published; construction then makes a
    /// second linear pass and links persistent Cons nodes in encoded order.
    fn lower_directory_entries_reply(
        &mut self,
        builder: &mut FunctionBuilder<'_>,
        response: SafeByteSpan,
        static_origin: StaticOriginId,
        seats: &ClaimedEffectSeats<'_>,
    ) -> Result<CarriedBoundaryWord, CraneliftBackendError> {
        use crate::cranelift_backend::planning::{
            PlannedAggregateShape, SynthesizedAggregateNode,
            SynthesizedAggregatePath, SynthesizedAggregateRoot,
            SynthesizedConstructorRole, SynthesizedFixedConstructorRole as Role,
        };
        use super::aggregates::CarrierAllocationRequest;

        let owner = self.defining_emission_owner.ok_or_else(|| {
            unsupported(
                "DirectoryEntries",
                "a response list is being decoded outside a declared emission owner",
            )
        })?;
        let root = SynthesizedAggregatePath::root(
            SynthesizedAggregateRoot::HostResultOk,
        );
        let alternatives = self
            .static_transition_plan
            .synthesized_dynamic_alternatives(static_origin, &root)?;
        let expected_alternatives = vec![
            SynthesizedConstructorRole::Fixed(Role::ListNil),
            SynthesizedConstructorRole::Fixed(Role::ListCons),
        ];
        if alternatives != expected_alternatives {
            return Err(unsupported(
                "DirectoryEntries",
                "the response-list alternative population is not exactly Nil/Cons",
            ));
        }
        let cons_path = root.alternative(1);
        let cons_role = SynthesizedConstructorRole::Fixed(Role::ListCons);
        let cons_children = self
            .static_transition_plan
        ...
        let cons_children_match = matches!(
            cons_children,
            [
        ...
        if !cons_children_match {
            return Err(unsupported(
                "DirectoryEntries",
                "the response Cons node is not planned as DirEntry plus a persistent loop link",
            ));
        }
        let cons_occurrence = self
            .static_transition_plan
        ...
        let cons_identity = self
            .synthesized_fixed_identity(Role::ListCons)?
            .tag_abi_word()?;

        let pointer = response.pointer();
        let length = response.len();
        Self::require_nonzero(builder, pointer);
        let has_header = builder.ins().icmp_imm(
            cranelift_codegen::ir::condcodes::IntCC::UnsignedGreaterThanOrEqual,
            length,
            8,
        );
        Self::require_nonzero(builder, has_header);
        let count = builder
            .ins()
            .load(types::I64, MemFlags::trusted(), pointer, 0);
        let after_header = builder.ins().iadd_imm(length, -8);
        let nine = builder.ins().iconst(types::I64, 9);
        let maximum_count = builder.ins().udiv(after_header, nine);
        let count_fits = builder.ins().icmp(
            cranelift_codegen::ir::condcodes::IntCC::UnsignedLessThanOrEqual,
            count,
            maximum_count,
        );
        Self::require_nonzero(builder, count_fits);

        // First pass: validate every length/kind and exact whole-payload
        // consumption before allocating any response-derived value.
        let validate = builder.create_block();
        builder.append_block_param(validate, types::I64);
        builder.append_block_param(validate, types::I64);
        let validate_body = builder.create_block();
        let validated = builder.create_block();
        builder.append_block_param(validated, types::I64);
        let zero = builder.ins().iconst(types::I64, 0);
        let header = builder.ins().iconst(types::I64, 8);
        builder
            .ins()
            .jump(validate, &[zero.into(), header.into()]);

        builder.switch_to_block(validate);
        let index = builder.block_params(validate)[0];
        let cursor = builder.block_params(validate)[1];
        ...
        builder
            .ins()
            .brif(more, validate_body, &[], validated, &[cursor.into()]);

        builder.switch_to_block(validate_body);
        let remaining = builder.ins().isub(length, cursor);
        let has_name_header = builder.ins().icmp_imm(
            cranelift_codegen::ir::condcodes::IntCC::UnsignedGreaterThanOrEqual,
            remaining,
            8,
        );
        Self::require_nonzero(builder, has_name_header);
        let name_header = builder.ins().iadd(pointer, cursor);
        let name_length = builder
            .ins()
            .load(types::I64, MemFlags::trusted(), name_header, 0);
        let name_start = builder.ins().iadd_imm(cursor, 8);
        let name_and_kind = builder.ins().isub(length, name_start);
        let has_kind = builder.ins().icmp_imm(
            cranelift_codegen::ir::condcodes::IntCC::UnsignedGreaterThanOrEqual,
            name_and_kind,
            1,
        );
        Self::require_nonzero(builder, has_kind);
        let available_name = builder.ins().iadd_imm(name_and_kind, -1);
        let name_fits = builder.ins().icmp(
            cranelift_codegen::ir::condcodes::IntCC::UnsignedLessThanOrEqual,
            name_length,
            available_name,
        );
        Self::require_nonzero(builder, name_fits);
        let kind_offset = builder.ins().iadd(name_start, name_length);
        let kind_pointer = builder.ins().iadd(pointer, kind_offset);
        let kind = builder
            .ins()
            .load(types::I8, MemFlags::trusted(), kind_pointer, 0);
        let kind = builder.ins().uextend(types::I64, kind);
        let kind_fits = builder.ins().icmp_imm(
            cranelift_codegen::ir::condcodes::IntCC::UnsignedLessThanOrEqual,
            kind,
            3,
        );
        Self::require_nonzero(builder, kind_fits);
        let next_cursor = builder.ins().iadd_imm(kind_offset, 1);
        let next_index = builder.ins().iadd_imm(index, 1);
        builder
            .ins()
            .jump(validate, &[next_index.into(), next_cursor.into()]);

        builder.switch_to_block(validated);
        let consumed = builder.block_params(validated)[0];
        let consumed_all = builder.ins().icmp(
            cranelift_codegen::ir::condcodes::IntCC::Equal,
            consumed,
            length,
        );
        Self::require_nonzero(builder, consumed_all);

        let nil = self.synthesized_constructor(
            static_origin,
            &root.alternative(0),
            Role::ListNil,
            self.process_symbols.list_nil.clone(),
            Vec::new(),
            seats,
        )?;
        let nil = self.emit_carrier_transfer(builder, static_origin, &nil)?;

        // Second pass: construct in encoded order. A Cons tail is written when
        // its successor becomes available; no incomplete node is observable
        // before the final HostResult is returned.
        let build = builder.create_block();
        for _ in 0..4 {
            builder.append_block_param(build, types::I64);
        }
        let build_body = builder.create_block();
        let build_done = builder.create_block();
        for _ in 0..2 {
            builder.append_block_param(build_done, types::I64);
        }
        builder.ins().jump(
            build,
            &[zero.into(), header.into(), zero.into(), zero.into()],
        );

        builder.switch_to_block(build);
        let index = builder.block_params(build)[0];
        let cursor = builder.block_params(build)[1];
        let head = builder.block_params(build)[2];
        let previous = builder.block_params(build)[3];
        let more = builder.ins().icmp(
            cranelift_codegen::ir::condcodes::IntCC::UnsignedLessThan,
            index,
            count,
        ...
        builder.ins().brif(
            ...
            &[],
            build_done,
            &[head.into(), previous.into()],
        );

        builder.switch_to_block(build_body);
        let name_header = builder.ins().iadd(pointer, cursor);
        let name_length = builder
            .ins()
            .load(types::I64, MemFlags::trusted(), name_header, 0);
        let name_start = builder.ins().iadd_imm(cursor, 8);
        let name_pointer = builder.ins().iadd(pointer, name_start);
        let kind_offset = builder.ins().iadd(name_start, name_length);
        let kind_pointer = builder.ins().iadd(pointer, kind_offset);
        let kind = builder
            .ins()
            .load(types::I8, MemFlags::trusted(), kind_pointer, 0);
        let kind = builder.ins().uextend(types::I64, kind);
        let kind_root = cons_path.field(0).field(1);
        let kind_value = Lowered::DynamicConstructor(DynamicConstructorV1 {
            discriminator: kind,
            alternatives: vec![
        ...
        let name = Lowered::ResponseBytes(
            response.rebuild_from_collected(name_pointer, name_length),
        );
        #[cfg(test)]
        let name_argument = if effect_seat_dispatch_mutation()
            == EffectSeatDispatchMutation::HostResponseAsScalar
        {
            SynthesizedArgument::Scalar(name)
        } else {
        ...
        #[cfg(not(test))]
        let name_argument = SynthesizedArgument::HostResponseReferent {
            class: BoundaryClass::Bytes,
            value: name,
        };
        let entry = self.synthesized_constructor(
            static_origin,
            &cons_path.field(0),
            Role::DirEntry,
            self.process_symbols.dir_entry.clone(),
            vec![
                name_argument,
                SynthesizedArgument::Dynamic(kind_value),
            ],
            seats,
        )?;
        let entry = self.emit_carrier_transfer(builder, static_origin, &entry)?;
        let cons = self.emit_carrier_alloc(
            builder,
            CarrierAllocationRequest::PlannedAggregate {
                occurrence: cons_occurrence,
                shape: PlannedAggregateShape::Constructor,
            },
            BoundaryClass::Constructor,
            2,
        )?;
        self.emit_carrier_store_tag_id(builder, cons, cons_identity)?;
        self.emit_carrier_store_field(builder, cons, 0, entry)?;

        let first = builder.ins().icmp_imm(
            cranelift_codegen::ir::condcodes::IntCC::Equal,
            index,
            0,
        );
        let first_entry = builder.create_block();
        let later_entry = builder.create_block();
        let linked = builder.create_block();
        builder.append_block_param(linked, types::I64);
        builder.append_block_param(linked, types::I64);
        builder
            .ins()
            .brif(first, first_entry, &[], later_entry, &[]);

        builder.switch_to_block(first_entry);
        builder
            .ins()
            .jump(linked, &[cons.word.into(), cons.word.into()]);

        builder.switch_to_block(later_entry);
        self.emit_carrier_store_field(
            builder,
            CarriedBoundaryWord { word: previous },
            1,
            cons,
        )?;
        builder
            .ins()
            .jump(linked, &[head.into(), cons.word.into()]);

        builder.switch_to_block(linked);
        let head = builder.block_params(linked)[0];
        let previous = builder.block_params(linked)[1];
        let next_index = builder.ins().iadd_imm(index, 1);
        let next_cursor = builder.ins().iadd_imm(kind_offset, 1);
        builder.ins().jump(
            build,
        ...
        builder.switch_to_block(build_done);
        let head = builder.block_params(build_done)[0];
        let previous = builder.block_params(build_done)[1];
        let empty = builder.ins().icmp_imm(
            cranelift_codegen::ir::condcodes::IntCC::Equal,
            count,
            0,
        );
        let empty_list = builder.create_block();
        let nonempty_list = builder.create_block();
        let complete = builder.create_block();
        builder.append_block_param(complete, types::I64);
        builder
            .ins()
            .brif(empty, empty_list, &[], nonempty_list, &[]);

        builder.switch_to_block(empty_list);
        builder.ins().jump(complete, &[nil.word.into()]);

        builder.switch_to_block(nonempty_list);
        self.emit_carrier_store_field(
            builder,
            CarriedBoundaryWord { word: previous },
            1,
        ...
        builder.ins().jump(complete, &[head.into()]);

        builder.switch_to_block(complete);
        Ok(CarriedBoundaryWord {
            word: builder.block_params(complete)[0],
        })
    }
    ...
    pub(super) fn lower_process_host_effect(
        ...
        if !CRANELIFT_HOST_EFFECT_CONSUMERS_V1.contains(&operation) {
            ...
            // the identical defect, one check earlier. The original governed
            // fixtures used the four ABI-A3 directory arms; after their
            // promotion the retained pair uses ClockMonotonicNow.
            //
        ...
        match operation {
            ken_host::HostOpV1::ConsoleRead => {
                if capability.is_some() {
                    return Err(unsupported(
                        "Effect",
                        "ambient ConsoleRead carried a capability",
                    ));
                }
                let stream = self.wire_constructor_tag_seat(
                    ...
                    SEAT_0,
                    console_stream_tag,
                    "ConsoleRead has a malformed Stream operand",
                )?;
                let limit = seats.specialized(SEAT_1)?;
                let (limit, valid) = self.narrow_native_int_u64(builder, limit)?;
                ...
                let detail = builder
                    .ins()
                    .iconst(types::I64, IO_ERROR_INVALID_INPUT_DISCRIMINATOR);
                ...
                builder
                    .ins()
                    .stack_store(limit, request, request_offset(1));
            }
            ...
            | ken_host::HostOpV1::FsRemoveDirectory => {
                ...
                let recursive_operation = matches!(
                    operation,
                    ken_host::HostOpV1::FsCreateDirectory
                        | ken_host::HostOpV1::FsRemoveDirectory
                );
                let path_slot = if recursive_operation { SEAT_1 } else { SEAT_0 };
                let path = self.wire_bytes_seat(builder, &seats, path_slot)?;
                if let Some((invalid, resource_code)) = path.refusal {
                    let detail = io_error_other_detail(builder, resource_code);
                    record_narrow_failure(builder, invalid, error_reply_tag, detail);
                }
                if recursive_operation {
                    let recursive = self.wire_constructor_tag_seat(
                        builder,
                        &seats,
                        SEAT_0,
                        bool_tag,
                        "FS directory operation has a malformed recursive Bool",
                    )?;
                    builder
                        .ins()
                        .stack_store(recursive, request, request_offset(1));
                    builder
                        .ins()
                        .stack_store(path.pointer, request, request_offset(2));
                    builder
                        .ins()
                        .stack_store(path.len, request, request_offset(3));
                } else {
                    builder
                        .ins()
            ...
            | ken_host::HostOpV1::FsOpen => {
                ...
                if operation == ken_host::HostOpV1::FsWriteFile {
                ...
                } else if operation == ken_host::HostOpV1::FsAppendFile {
                    let contents = self.wire_bytes_seat(builder, &seats, SEAT_1)?;
                    if let Some((invalid, resource_code)) = contents.refusal {
                        let detail = io_error_other_detail(builder, resource_code);
                    ...
                    builder
                        .ins()
                        .stack_store(contents.pointer, request, request_offset(3));
                    builder
                        .ins()
        ...
        if operation == ken_host::HostOpV1::ConsoleIsTerminal {
            ...
            let success_tag = match operation {
                ...
                ken_host::HostOpV1::FsReadFile | ken_host::HostOpV1::FsReadDirectory => {
            ...
            let accepted_tags = match operation {
                ken_host::HostOpV1::ConsoleRead => vec![
                    wire.reply_bytes_tag as i64,
                    wire.reply_unit_tag as i64,
            ...
            let error = if matches!(
                ...
                let (operation_role, operation_symbol) = match operation {
                    ...
                    ken_host::HostOpV1::FsReadDirectory => (
                        SynthesizedFixedConstructorRole::FileOperationReadDirectory,
                        self.process_symbols.file_operation_read_directory.clone(),
                    ),
                    ken_host::HostOpV1::FsCreateDirectory => (
                        SynthesizedFixedConstructorRole::FileOperationCreateDirectory,
                        self.process_symbols.file_operation_create_directory.clone(),
                    ),
                    ken_host::HostOpV1::FsRemoveFile => (
                        SynthesizedFixedConstructorRole::FileOperationRemoveFile,
                        self.process_symbols.file_operation_remove_file.clone(),
                    ),
                    ken_host::HostOpV1::FsRemoveDirectory => (
                        SynthesizedFixedConstructorRole::FileOperationRemoveDirectory,
                        self.process_symbols.file_operation_remove_directory.clone(),
                    ),
                    ken_host::HostOpV1::FsRename => (
                ...
                let operation_value = self.synthesized_constructor(
                    static_origin,
                    &error_root.field(0),
                    operation_role,
                    operation_symbol,
                ...
                let path_argument = self.site_operand_argument(
                    builder,
                    static_origin,
                    path_ordinal,
                    &seats,
                ...
                self.synthesized_constructor(
                    ...
                    vec![
                        SynthesizedArgument::Nested(operation_value),
            ...
            let success = if operation == ken_host::HostOpV1::ConsoleRead {
            ...
            if operation == ken_host::HostOpV1::FsReadDirectory {
                let response = masked_reply_response_bytes(
                    builder,
                    pointer_type,
                    reply,
                    wire.reply_bytes_data_offset,
                ...
                let Lowered::ResponseBytes(response) = response else {
                    unreachable!("masked reply bytes always produce a SafeByteSpan")
                };
                ...
                let entries = self.lower_directory_entries_reply(
                    builder,
                    response,
                    static_origin,
                    &seats,
                )?;
                builder.ins().jump(selected, &[entries.word.into()]);

                builder.switch_to_block(error_block);
                let error = self.emit_carrier_transfer(
                    builder,
                    static_origin,
                    &error,
                )?;
                builder.ins().jump(selected, &[error.word.into()]);

                builder.switch_to_block(selected);
                let selected_value = CarriedBoundaryWord {
                    word: builder.block_params(selected)[0],
                };
                let success_word = builder.ins().uextend(types::I64, success);
                let result = self.emit_carrier_alloc(
                    builder,
                    super::aggregates::CarrierAllocationRequest::NonAggregate {
                        tag: BoundaryTag::InvocationHostResult,
                    },
                    BoundaryClass::HostResult,
                    1,
                )?;
                self.emit_carrier_store_scalar(builder, result, success_word)?;
                self.emit_carrier_store_field(builder, result, 0, selected_value)?;
                return Ok(LoweringOperand::Carried(result));
            }
            let ok = if operation == ken_host::HostOpV1::ConsoleRead {
                let is_chunk = builder.ins().icmp_imm(
                    cranelift_codegen::ir::condcodes::IntCC::Equal,
                    tag,
                    wire.reply_bytes_tag as i64,
                );
                let is_eof = builder.ins().icmp_imm(
                    cranelift_codegen::ir::condcodes::IntCC::Equal,
                    tag,
                    wire.reply_unit_tag as i64,
                );
                ...
                #[cfg(test)]
                let response_argument = if effect_seat_dispatch_mutation()
                    == EffectSeatDispatchMutation::HostResponseAsScalar
                {
                    SynthesizedArgument::Scalar(response)
                } else {
                    SynthesizedArgument::HostResponseReferent {
                        class: BoundaryClass::Bytes,
                        value: response,
                ...
                #[cfg(not(test))]
                let response_argument = SynthesizedArgument::HostResponseReferent {
                    class: BoundaryClass::Bytes,
                    value: response,
                };
                Lowered::DynamicConstructor(DynamicConstructorV1 {
                    discriminator: builder.ins().uextend(types::I64, is_eof),
                    alternatives: vec![
                        self.synthesized_dynamic_alternative(
                            static_origin,
                            &ok_root,
                            ...
                            self.process_symbols.read_chunk.clone(),
                            vec![response_argument],
                            &seats,
                        )?,
                        self.synthesized_dynamic_alternative(
                            static_origin,
                            ...
                            self.process_symbols.read_result_eof.clone(),
                            Vec::new(),
                            &seats,
                        )?,
                    ],
                })
            } else if operation == ken_host::HostOpV1::FsReadFile {
