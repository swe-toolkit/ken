...
pub fn with_d5b_hs3_call_mutation<T>(
    ...
    let restore = Restore;
    let result = body();
    let observations = D5B_HS3_CALL_OBSERVATIONS.with(|observations| observations.borrow().clone());
...
#[cfg(test)]
pub(in crate::cranelift_backend) fn d5_emitted_declaration_calls() -> Vec<(
    StaticOriginId,
    StaticOriginId,
...
impl<'a> Lowering<'a> {
    ...
    pub(super) fn materialize_checked_ih_static_worker_application(
        ...
        match pending.kind {
            ...
            crate::CheckedComputationalIHInvocationKind::CheckedHostVisContinuation => Ok(None),
        }
    }
...
impl<'a> Lowering<'a> {
    ...
    fn resolve_declared_recursive_position_call(
        ...
        let result = match context {
            Some(context) => {
                self.call_declared_context(builder, context, body_origin, inputs, coordinates)?
            }
            None => {
...
impl<'a> Lowering<'a> {
    ...
    fn call_declared_context(
        ...
        if view.worker_body_origin() != body_origin || unit.worker_body_origin() != body_origin {
            return Err(unsupported(
                "ContinuationSpecialization",
                format!(
        ...
        let caller_requires_frame_workers =
            matches!(&inputs, RecursivePositionCallInputs::DeclaredArguments(_))
        ...
        if _complete_direct_worker
            && d5b_hs3_call_mutation() == D5bHs3CallMutation::ReappendFrameWorkerCaptures
        {
        ...
        if context_requires_frame {
            ...
            let context_captures = {
                ...
                if _complete_direct_worker
                    && d5b_hs3_call_mutation() == D5bHs3CallMutation::SuppressFrameContextCaptures
                {
                    record_d5b_hs3_call_mutation_application();
...
impl<'a> Lowering<'a> {
    ...
    pub(super) fn transfer_unit_result_into_carrier(
        ...
        let result_identity = match value {
            Lowered::Constructor {
                synthesized_identity: Some(identity),
                ..
            } => Some(*identity),
            _ => None,
        };
        let word = if process_exit {
