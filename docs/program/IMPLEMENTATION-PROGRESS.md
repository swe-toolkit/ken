...
2026-08-13 18:18:27Z — from 237 issue file(s) in `docs/program/issues/`.
...
| `LANG-SURFACE-LITERAL-ESCAPES` | The lexer performs NO escape processing -- its single string form pushes every character verbatim, so the escape repertoire that SPEC-LITERAL-ESCAPE-PIN just closed is entirely unimplemented, and Char literals, byte literals, byte strings and raw triple strings do not exist at all despite String, Char and Bytes all being built prelude targets | ready | language | M | none | — |
...
| `RT-DYNAMIC-ARM-SCALAR-MERGE` | A carried Match arm carrying a nested-IH result cannot satisfy merge_scalar_operand -- measure what the arm actually produces before bounding the repair | ready | runtime | M | none | — |
...
| `RT-NESTED-IH-NATIVE-REALIZATION` | Native realization of the nested-IH recursive computation beyond scalar admission -- emitted definition, ABI/owner wiring, and execution that survives the Cranelift verifier and agrees with the interpreter at Nat 3 | draft | runtime | L | none | — |
...
- `RT-4B-UNIQUENESS-GATE-REACH` blocked by `RT-4B-C2-REACHABILITY` (status: active)
