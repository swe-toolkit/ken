...
> # HELD — release-ordered behind WP-2 D1. Buildable now (enclave GO'd).
>
> Architect sequence: NODE A -> [[LANG-MOD-STRICT-RESOLUTION]] D1 -> NODE B
> ([[LANG-MOD-OR-CANONICAL-HOME]]). NODE A and WP-2 D1 are mutually independent
> (this touches the data elaborator; D1 touches resolution) and both must precede
> NODE B. The language ring is single-threaded, so NODE A releases when the ring
> frees from WP-2 D1. Not on any other lane's critical path.
