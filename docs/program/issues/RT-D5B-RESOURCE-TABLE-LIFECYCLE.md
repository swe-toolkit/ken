...
> lane reaches it.
>
> **Its dependency is SATISFIED as of 2026-09-16.**
> `RT-D5B-HOST-FILE-ACQUISITION-SURFACE` (slice 4) reads `status: merged` on
> `main`, so the re-measure this node's `origin` asks for is now *possible* —
> the backup-vs-`main` delta can be taken against a tree that already contains
> slice 4. **Possible is not done.** Take that measurement at framing time;
> the 47-hunk census in `origin` predates slice 4 landing and its cluster-C
> count of six is the number most likely to have moved.
