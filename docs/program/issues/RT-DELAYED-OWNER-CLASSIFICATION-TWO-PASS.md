...
Do not re-derive any of this, and do not re-open it.

- **The shape, the forced order, and the measured engine contrast** are in
  `RT-BRACKET-RELEASE-ORDER-PARITY` sections 2.1 through 2.5, measured at
  `89d2bfb57`. Native is wrong on all six reaching depth-2 nests and right on
  both reaching depth-3 nests; interp is correct on all four measured composed
  programs. **There is no measured interp defect and no interp repair is
  authorized.**
- **The cause** is planner classification, not depth: Specialized or
  handler-owned execution versus unowned Deferred forward-`Ret`
  (`evt_4t14zmba83hjm`).
- **Both arms are required, and that is measured, not asserted**
  (`evt_76nkdg0h81xnw`). Arm A alone leaves px8ta wrong; Arm B alone leaves the
  composed family wrong **and cannot clear px8ta at depth 3**
  (`evt_493eky78hsp3h`).
- **Dead leads, already disproved as sufficient — do not re-propose them:**
  mode, inner-kind, outer-kind, homogeneity, combinator, read-vs-write,
  `ordinary_continuation_call_identities`, checked-IH transport membership,
  group shape, operation kind.
- **A PASS THROUGH THE BASE ORACLE IS NOT A RELEASE-ORDER MEASUREMENT.**
  `assert_narrowed_alike` (`rt_parity_native.rs:632`) compares bracket releases
  **as a set with relative order excluded**. That is the exclusion `D3` removes.
  `fs_read_at_malformed_window_narrows_to_invalid_bounds` has **no release-order
  observation on either tree** — it is the fixture that broke, and it is the one
  no ordered reading has ever covered.
...
**D3 — remove the order-excluding helpers and re-enable release-order parity on
the five section 2.3 fixtures, once both engines agree.**

**D4 — atomic closure of `RT-DEPTH3-CONTINUATION-CLAIM-UNDECLARED` in the same
candidate.**

## Acceptance criteria

**AC-1. No `#[ignore]` attribute is added anywhere.** Count attributes, never
reason strings.

**AC-2. `px8ta public_two_three_level_brackets_finish_and_release_lifo` passes
at BOTH depth 2 and depth 3, with strict LIFO at each, and is no longer
`#[ignore]`d.** Its stale reason string is deleted, not edited. **There must be
no `ContinuationSpecialization` object-emission refusal at depth 3, and no
refusal of any other kind.**

**AC-3. Report the observed release SEQUENCE before and after, as vectors**, for
every fixture named in `AC-7`. A set comparison is not evidence.

**AC-4. Both `D2a` controls exist, are not `#[ignore]`d, and pass** — `D2a-A` a
depth-2 end-to-end control for the composed single-stage arm, `D2a-B` a distinct
depth-2 px8ta control for the release-only suffix arm. **The px8ta
depth-2-plus-depth-3 loop row is not this arm's control:** it exercises both
arms at once and a loop body reports nothing past its first failure.

**AC-5. Mutation-prove the two arms INDEPENDENTLY**, plus the witness
mutations below. One aggregate release-order test is not evidence for both.

**AC-6. All three `D2b` depth-3 causal controls pass on the UNCHANGED px8ta
program** — `RestoreSelectedKTarget`, `OmitKCall`, and Arm-A suppression
retained as route-replacement evidence.

**AC-7. No composed-return fixture regresses, and each is named individually.**
...
`fs_read_at_malformed_window_narrows_to_invalid_bounds`, and
...
fatal semantics and its exact missing-selected-caller diagnostic string. **Show
the diagnostic text is byte-identical to current `main`.**

**AC-11. No-regression in CI**, per `COORDINATION §12` — green in CI, never a
local `--workspace` run.

**AC-12. CLOSE OBLIGATION, CLEARED-ROW BRANCH.** This node does not reach
`merged` unless `AC-2` holds and `RT-DEPTH3-CONTINUATION-CLAIM-UNDECLARED` moves
...
changes to the existing final closeouts, reordering release events after
emission, host-dispatch reorder, a `ResourceRelease` kind split, exploratory
logging, global diagnostic recoding, or any capacity change. **A fourth
carve-out is not authorized and never was.**
