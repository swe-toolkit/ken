...
> #
> # **What outcome C settled, and where this starts.** The live `Vis e k` binds
> # the ordinary continuation Function, NOT the capsule endpoint: read runs
> # application138 into unit2/body941 (then unit3/body452/Match451/trap36); write
> # runs application175 into unit5/body1259 (then unit6/body465/Match464/trap37).
> # Applications146/159 are off-chain alternatives that hit ZERO, and
> # body662/body888 are reached by no call the live continuation unit emits. So
> # the capsule endpoint route is not selected at the live request boundary. This
> # D0 asks the next question INSIDE the live `k`: from the HostResult entry of
> # unit2/body941 and unit5/body1259, is the body662/body888 executor route
> # reachable from live source control at all? **Start inside the live `k`, at the
> # exact HostResult entry and the proven downstream calls to body452/body465.
> # Do NOT start at applications146/159 or at an environment transport, and do NOT
> # substitute the entry HostResult.**

## What this D0 does

For BOTH read (unit2/body941) and write (unit5/body1259) witnesses, using NO
body/table/shape/count/proximity inference, no environment-as-result, no
synthetic success, and no early call:

1. **Enumerate the selected source-control path** inside the live `k`, from the
   entry HostResult to its downstream result consumer (the proven
   body452/body465 edge). This is the ordered source-control spine the response
   actually travels.

2. **On that path, identify every checked computational-IH application whose
   binder could name the body662/body888 worker.** These are the only candidate
   sites at which a later production design could stage the affine call. Names
   come from the source relation, never from body/table presence, environment
   shape, runtime word, order, or numeric proximity.

3. **For each candidate application, derive independently:** the source `Call`;
   the binder/slot authority; the explicit argument producer and its type; the
   Function-local emission owner; the exact declared target route; the ordered
   captures; and the downstream result consumer.

## Expectation and actual use must be independent

The **fixed expectation** is the source application's binder/slot authority and
its explicit argument demand. The **actual use** is the real emitted call target
and the real argument edge. These two must be derived from independent
representations — a control that rebuilds one from the other (as the prior
result-producer clone did) is detector-side and proves nothing.

## Mutation controls

- **Mutate the actual target and the actual argument producer SEPARATELY**
  against unchanged expectations; each must be refused at the distinct authority
  disagreement, not at a cardinality/absence diagnostic.
- **For any redirect control, preserve the consumer selection key and cardinality
  one, replacing only the payload** from a structurally derived same-Match /
  different-alternative sibling at equal binding, candidate-unit set, and arity —
  the exact repair the VIS-RESPONSE recut established (do not key the mutation on
  the consumer's own selection field, which collapses redirect to drop).
- **Drop and duplicate remain separate** required properties, each reaching the
  exact cardinality refusal.
- **Prove mutation provenance and restore an independent positive.**

## Preservation invariants

Preserve Trap-before-Result, effect order, planned identities36/37, the live
applications138/175, the capsule zeros, the accepted CLIF/runtime net
byte-identically, and the 474/473 zero-argument no-call materializer. Restore the
branch byte-clean at the end.

## Exhaustive routing — land in exactly one

- **A.** One selected inner source application in each live `k` independently
  binds body662/body888 and its explicit argument. Report the exact application
  and argument provenance; only then may a later production design stage the
  affine call at that source application.
- **B.** A unique application exists, but its explicit argument is produced only
  after intermediate source computation. Report the ordered producer chain and
  the first missing edge; the next design begins there and may NOT substitute the
  entry HostResult.
- **C.** No selected source application binds body662/body888. Retire the
  body662/body888 executor route for this defect and return to the ordinary
  body941/body1259 result-supply edge feeding body452/body465.
- **D.** Candidate applications exist but binding, target, or argument provenance
  is ambiguous or not independently represented. Stop and name that exact
  representation seam.
...
reviews it and rules on the production design. This completes into hard stop 8 if
the successor is failing or absent; the next Research advisory trigger remains
hard stop 9. Runtime production stays blocked until this successor is ruled.
