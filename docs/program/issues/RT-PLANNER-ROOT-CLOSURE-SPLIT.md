...
1. **The parent is already under 10k.** ⇒ **Record that and do no speculative
   extraction.** This is a complete, mergeable result. The operator's constraint
   is satisfied and there is nothing to move.
...
this slice just established** (any planner-root controls left unattributed by
items 4-9). Place multi-leaf fixtures **once**,
