...
> ### D1a BLOCKER CLEARED — 2026-08-20. [[LANG-CTOR-PREMISE-ELABORATION-DIVERGES]] MERGED.
>
> **The kernel positivity fix landed** (LANG-CTOR D2 at `80630656f`; node closed
> `merged` at `9507334fc`). The strict-positivity check no longer normalizes a
> constructor argument to a full normal form — it exposes the head with `whnf`
> and the occurrence check is delta-aware (`occurs_delta`), so a constructor
> premise applying a recursive function to a telescope-bound variable terminates
> instead of diverging. `D1a` is therefore **dispatchable**. The `D1b` chain
> below descends through `D1a`, so the node now has **no remaining blocker** —
> the "EXACTLY ONE BLOCKER" framing below is superseded. The original blocked
> analysis is retained for the record.
>
