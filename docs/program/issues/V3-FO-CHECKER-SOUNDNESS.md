...
> **`D0` split this deliverable in half, and only the second half is blocked.**
>
> - **`D1a` — `FokDerivation` itself. DISPATCHABLE NOW.** `D0` part (1) proved
>   the indexed-family form elaborates and kernel-checks over a user-defined
>   index. Nothing about the rule set needs truncation, and `AC-2`'s per-rule
>   mutation control is discharged here.
...
### `D3`/`D4` are not affected, and `D2` is dispatchable today
...
⇒ **This node is not stalled. `D1a` and `D2` are both dispatchable while
[[LANG-TRUNCATION-SURFACE-SYNTAX]] is built.**
