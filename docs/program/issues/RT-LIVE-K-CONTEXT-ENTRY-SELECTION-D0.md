...
> # CLOSED 2026-08-31 — HARD STOP 11 (Architect review evt_6j7x9refa7m8x, thr_3s8j).
> #
> # MEASUREMENT ACCEPTED. The Architect reproduced all seven primary hashes and
> # all 96 evidence members, applied the scratch to 551de8084 (regenerated binary
> # diff hash matches), ran an independent `scripts/ken-cargo check -p ken-runtime
> # --features px8-ds-test-support` green, and confirmed the disabled 10/10 read and
> # 18/18 write CLIF corpora are byte-identical to the clean base (both recursive
> # diffs empty). WRITE is C at the FIRST LIVE PREDECESSOR: live app486/u0:53 has no
> # emitted selector/use that enters context0/u3:57 — the context target declaration
> # (fn39/fn41 to u0:57, 19 resolved FuncRefs, zero call/return uses) is a
> # DECLARATION, not execution authority. The empty selector population is honest
> # CONTROL_NO_SUBJECT; the planner-edge mutations get no credit. The detector-
> # independent READ controls are valid — the exact (owner, final-use) matrix is
> # baseline (1,1) / owner-drop (0,1) / owner-dup (2,1) / final-drop (1,0) /
> # final-dup (1,2), each restored (1,1) — accepting the natural READ observation
> # only (producer705 completes 0x0305, correctly owned, nested in env 0x0e09, and
> # the body452 position1 use forwards that env container), NOT a projection.
> #
> # ROUTING-HEADLINE CORRECTION (Architect, binding): context0 entry is NOT "proven
> # via a live incoming edge" — its marker is ZERO and there is no incoming selector.
> # What is proven is that the SOURCE side of the expected edge is live and the edge
> # is ABSENT. Do not propagate the "proven via a live incoming edge" wording (the
> # Steward's earlier routing line and lane note carried it; corrected).
> #
> # SYMPTOM-INVENTORY ENTRY 11 (verbatim): the live write application materializes
> # body1238's environment, but no planner-issued context selector is consumed;
> # context0 is declared in the live Function and remains unentered.
> #
> # SUCCESSOR: RT-LIVE-K-CONTEXT-SELECTOR-AUTHORITY-D0 (ready, scratch-only, same
> # accepted runtime net) — DO NOT mint a new capsule/call. Ken already has a typed
> # context-selection mechanism on the READ path (`call_declared_recursive_position_unit`
> # resolves `CarriedInvocationCoordinates` through `carried_invocation_context`, emits
> # `call_declared_context`; the finished READ corpus has 14 calls to context0/u0:50,
> # WRITE has zero uses of context0/u0:57). Subsume against that mechanism and locate
> # the phase divergence. HS9 Research advisory covers the predicate; no new mandatory
> # advisory at HS11. Any result other than a genuinely-live A at a proven-live
> # coordinate is HARD STOP 12. The original READY frame follows for the record.
> #
> # ---- ORIGINAL READY FRAME (below) ----
>
