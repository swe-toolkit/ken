...
> # CLOSED 2026-09-11 (Steward) — confirm-or-refute DISCHARGED; framed walker-fix
> # EXCLUDED with grounds; route-(a) fix carved to [[LANG-TRANSPORT-SIGMA-PREMISE-
> # SYNTHESIS]] (deferred). No code landed — the deliverable WAS the confirm-or-
> # refute, and the node's own "record EXCLUDED with a cited reason" is the terminal
> # branch taken.
> #
> # OUTCOME (Architect ruling evt_79xy16y7fv5sx, re-grounded at b148a96b):
> #  - REACHABLE: CONFIRMED. The compound case exists — a result family indexed by
> #    BOTH the whole matched record AND a bare component of it, co-occurring in one
> #    recursive-group-call result. The implementer's walker-in-transport experiment
> #    reproduced the first-index-only refinement (the confirming measurement).
> #  - The FRAMED FIX ("apply the walker exactly as the parent did") is REFUTED with
> #    grounds: transport builds a SINGLE raw Eq(scrut_ty, concrete, scrut_core) over
> #    the matched USER data record (elab.rs ~4881-4890), and obs::eq_reduce reduces
> #    Eq to a Sigma only for a structural Term::Sigma, never a user data IndFormer —
> #    so the walker gets exactly one whole-record leaf. transport is the LONE
> #    outlier consumer; every working consumer builds its premise as a Sigma-
> #    telescope via synth_generated_index_evidence (~1584) decomposed by the walker
> #    (~1672). Direction: COMPLETENESS gap, not soundness — KernelRejected
> #    TypeMismatch on a well-typed fixture (over-strict refusal), kernel/TCB
> #    untouched, deferral legitimate.
> #  - The correct fix is route (a): reshape transport's premise into the Sigma-
> #    shaped result-family-index equality and feed the existing walker (REUSE, not
> #    new capability; standalone raw-record injectivity decomposition REJECTED). It
> #    EXCEEDS the confirm-or-refute this WP was sized for (Architect ground iii), so
> #    it is a separate node, DEFERRED (safe completeness gap, narrow/exotic trigger).
> #  - Symptom inventory entry 1 (hard-stop #1 on this design question; no research
> #    pull until the 3rd) carried to the successor node.
> #
