...
# CLOSED — 2026-08-22 (accepted partial; SCT-pass half spun out and re-pointed)
...
ruling authorized (`evt_4q9gwy44nzf1d`) — this node's transport deliverable is
done.

**Full admission is `kernel_check` AND `SCT-pass`.** This node delivered the
`kernel_check` half. The `SCT-pass` half — the gate that actually unblocks
`V3-FO-CHECKER-SOUNDNESS` D3 — was ruled to a SEPARATE, operator-gated kernel
SCT successor (route A, `sct.rs` telescope-canonicalization). The operator
authorized that successor on 2026-08-22 ("tcb change authorized. proceed."); it
is now cut and released as [[KERNEL-SCT-TELESCOPE-CANON]].

**Close sequence executed (2026-08-22), in the order the hazard requires:**
(1) cut `KERNEL-SCT-TELESCOPE-CANON`; (2) re-point
`V3-FO-CHECKER-SOUNDNESS`'s `depends_on` from this node to
`KERNEL-SCT-TELESCOPE-CANON` (so this close does not falsely unblock V3-FO — the
`gen-progress` `closed`==`merged` clearance hazard); (3) close this node. D3
resumes on `KERNEL-SCT-TELESCOPE-CANON`'s landing.
