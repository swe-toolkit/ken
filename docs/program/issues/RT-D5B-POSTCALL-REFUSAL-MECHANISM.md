...
>     (B) subject is a core.rs EMITTER          THREE candidates, all REGION 3, none
>                                               carried by this port. Blocker is Region 3,
>                                               and the flip node would never discharge it.
>     (C) the grant gates reachability          the Steward's original edge. TRUE for
>                                               the acceptance test, UNPROVEN in general.
>
> **(B)'s three candidates, resolved by enclosing function** (Architect
> `evt_37yddzr5z47xr`, correcting their own first naming of the weakest one).
> At prefix tip `30d35f625`, `lowering/core.rs` emits the string from **three**
> functions, not one:
>
>     :9090  fn checked_ih_post_call_residual              emits :9097
>     :9120  fn validate_checked_ih_consumed_active        emits :9127 :9169
>     :9181  fn validate_checked_ih_detached_result_shape  emits :9191 :9200
>
> ⇒ **Of the three, `checked_ih_post_call_residual` is the one that matches this
> node's subject** — `POST-CALL` is in the node id, in the title, and in the
> function name. **Not asserted**, and none of the three lands with this port.
>
> The retired coordinate `core.rs:7720` sits among these sites and what is there
> is the
...
>
> > **PRECONDITION — check this first, in one command.** This trigger is valid
> > only while `lowering/core.rs` on `main` carries **no**
> > `CheckedIhDetachedCallerCut` emitter. True at `origin/main` `3813ceed1`:
> > `git show origin/main:crates/ken-runtime/src/cranelift_backend/lowering/core.rs
> > | grep -c CheckedIhDetachedCallerCut` returns **0**. **If it returns nonzero,
> > STOP — this trigger no longer discriminates and must be re-derived.**
> >
> > **THE TRIGGER.** If the port's AC-1/AC-4 build produces a red that carries the
> > `CheckedIhDetachedCallerCut` Packaging reason **with the grant fully
> > excluded**, and that red **names `lowering/source.rs` as its emitting file**,
> > then reading (C) is refuted and reading (A) is **reachable**.
> >
> > **REACHABLE IS NOT SUBJECT, and only the first half is automatic.** A
> > `source.rs` red refutes (C) outright — the grant is not what gates
> > reachability — and **the flip dependency does not come back** on that
> > evidence. It does **not** by itself make (A) this node's subject: (B)'s
> > `checked_ih_post_call_residual` reading stays live, and promoting this node to
> > `ready` on a `source.rs` red while its actual site is a Region 3 function
> > absent from `main` **hands a puller a node whose premise is false** — the
> > defect the `check-issue-schema` hook caught on this very node, arriving by a
> > different door. **Re-`ready` requires naming which reading the red's emitting
> > FUNCTION supports. That is a Steward call, not an automatic flip.**
>
...
> classifying it, AND name the file and function it was emitted from.** The
> reason string alone answers reachability; the emitting function is what
> discriminates (A) from (B), and it costs one line. On the post-port tree it can
> only be `source.rs` — which is exactly why recording it matters, because that
> uniqueness is a property of **this** tree and expires the moment any Region 3
> port lands `core.rs`.
>
