...
| 6h | `RT-LEXICAL-CALL-ARG-WITNESS-OR-PORT` | M | **Filed 2026-08-16 from #7's `D1` hard stop** (Architect `evt_620806vfy5kwm`). `D1` found production still selecting the lane 31 times over 15 exact renderings; **12 of them, 27 compiles, carry `LexicalCallArgumentRecursor`**, whose own doc says the port was never built. **The disposition axis is the VARIANT, not the expression** — 15 renderings carry at most two dispositions. First deliverable is a per-expression **fixture-or-production triage**, the operator's own witness-or-invariant method applied to *selections* rather than *refusals*. **This node is also `#6d`'s gate repair:** `#6d` was accurate about the rows-1-5 scope it named, and the defect is the retirement gate that consumed a scoped claim as if it were variant-wide. **Whether the port is worth building is NOT ruled** — scope, decided after the triage |
| 6i | `RT-MATCH-SCRUTINEE-DISPOSITION` | S | **Filed 2026-08-16, same ruling.** The other 3 renderings / 4 compiles carry `MatchScrutineeRecursor`, and **the tree records no reason for it** — one line of doc against its neighbour's seven. **Unbuilt port or correct refusal is undetermined, and symmetry with #6h is explicitly not an argument**; the Architect declined to rule it by analogy. `D1` is establishing *why* the functionized lane does not take the shape; everything else is conditional on that answer |
| — | *#6h and #6i* | | **Both gate #7 and they are independent of each other.** Measurement `A`/`B` (2026-08-16) fixed both populations by hash and proved the split is a **true partition** — zero dual-retained renderings — despite the original `D1` probe having read the **short-circuiting** selector. **Use `observed_recursive_descent_residuals()`, never the selector.** `B` proved all fifteen singly retained, so an exclusion result is **sole-retainer evidence and not capability evidence** |
| 7 | `RT-DESCENT-RETIRE` | M | delete the selector, enum, authority and lane; bank the win. Gated on **seven** nodes — the four migration nodes, **#6**, and the two variant dispositions **#6h** and **#6i**. **Its `D1` hard-stopped 2026-08-16 exactly as section 7 of the frame required**: the lane is still selected by production, so nothing was deleted. That is the control working, not a setback |
