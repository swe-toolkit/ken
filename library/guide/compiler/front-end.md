...
validity, and the elaborator prepares terms and metadata for that authority.
...
an input route. A `.ken.md` path uses the literate-source route, and another path
...
uses the same input route, then asks the interpreter to evaluate an entry point, and
...
[grammar](../../../spec/30-surface/32-grammar.md) specifications are normative, and
...
global lookup, and it is not a general resolver-stage refusal. `resolve_expr_ctx`
...
Elaboration prepares a term for checking, and it is not the authority that makes a
...
the language contract, and this page explains the implementation route applying it.
