...
elaborates surface declarations, and the kernel checks the resulting core term
...
body, and the [substitution module](../../../crates/ken-kernel/src/subst.rs) supplies
...
position. A lookup outside that telescope returns no type, and `infer` is the later
...
`raw_well_formed` checks structural scoping only, and it does not establish that a
...
ascription redexes. A neutral application or eliminator can remain stuck, and
...
reduction and type-directed eta cases for functions and pairs, and `convert_type`
...
an inductive declaration, and it does not execute an eliminator.
...
arities, and when no iota reduct is available, `whnf` retains a stuck eliminator.
