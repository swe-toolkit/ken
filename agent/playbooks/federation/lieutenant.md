...
- **The Adversary hook (M8) for code merges.** Docs-only merges skip M8.
  Do the following, in order: (1) **important** run `moot compact adversary`
  (2) notify it naming the landed **squash** SHA (not the tip), the paths, and the
  shortstat, then rouse its pane.
