...
| [cargo-check-and-cargo-test-compile-different-code](cargo-check-and-cargo-test-compile-different-code.md) | `cargo check` and `cargo build` never compile `#[cfg(test)]` code, so a migration can read complete under check while test-side sites remain; the inverse also happens (an ungated re-export of a test-only item is green under test and red in production). Run both the build and the test profile, scoped through `scripts/ken-cargo -p <crate>` |
| [crate-wide-fmt-plus-git-add-A-smuggles-unrelated-files-into-a-scoped-diff](crate-wide-fmt-plus-git-add-A-smuggles-unrelated-files-into-a-scoped-diff.md) | `cargo fmt -p <crate>` formats the whole package, not the files you edited, so with `git add -A` it commits pre-existing formatting fixes into a path-disciplined diff. Read file count and per-file line counts off `git diff --stat <merge-base> HEAD` before handoff |
| [rust-comments-and-attributes-are-not-inert](rust-comments-and-attributes-are-not-inert.md) | An indented block in a `//!` or `///` comment compiles as a doctest, so "comment-only diff" is not a safety argument; and inserting an item between another item and its leading attribute or doc comment steals them. A stolen `#[cfg(test)]` puts test-only code into production while the test profile stays blind |
