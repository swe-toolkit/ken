...
| Lesson | One-line |
|---|---|
| [a-block-recorded-as-an-unresolved-decision-outlives-the-thing-it-blocked](a-block-recorded-as-an-unresolved-decision-outlives-the-thing-it-blocked.md) | Only BLOCKS can strand — an approval closes its own record, a block leaves the Decision `proposed` and a SUPERSEDING Decision does not close it, so "awaiting a recut" and "the Architect never voted" become the same state in the store, read by the Steward's watchdog and by the publisher path. Sweep `list_decisions(proposed)` on every resume, not just your mentions |
...
| [a-stated-impossibility-carries-a-qualifier-and-the-qualifier-is-the-escape](a-stated-impossibility-carries-a-qualifier-and-the-qualifier-is-the-escape.md) | "No INTEGRATION test can name it" is not "no test can name it" — verify the claim, then verify its SCOPE. Before hardening a coupling ask why it is a coupling, and treat a "nothing reds" guard as a diagnosis, not a fix |
...
| [an-absence-claim-is-refuted-at-the-consumer-not-where-the-subject-is-defined](an-absence-claim-is-refuted-at-the-consumer-not-where-the-subject-is-defined.md) | Before writing "unmeasured", grep the subject in the file you are ALREADY citing — a "does X reach Y" fact is recorded at Y, where it is REFUSED, not where X is defined or exercised; and when the question is "what still guards this?", census every consumer of the observable |
...
| [reference-derived-corroboration-must-not-become-a-frames-cited-rationale](reference-derived-corroboration-must-not-become-a-frames-cited-rationale.md) | The clean-room boundary is crossed by the frame, not by an authorized enclave read: prior art may corroborate a conclusion reached from Ken's own evidence, but written into a WP frame as its rationale it becomes reference-derived design an implementer has no licence to check |
